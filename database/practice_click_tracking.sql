...
create or replace function public.log_student_practice_click(
  p_student_id text, p_grade integer, p_full_name text,
  p_learner_code text, p_practice_id text, p_click_kind text
...
begin
  ...
  where p.id::text=p_practice_id and p.student_id::text=p_student_id
    and p.grade_level=p_grade and p.active=true
  limit 1;
  if not found then raise exception 'Practice item unavailable'; end if;
end $$;
revoke all on function public.log_student_practice_click(text,integer,text,text,text,text)
  from public, anon, authenticated;
grant execute on function public.log_student_practice_click(text,integer,text,text,text,text)
